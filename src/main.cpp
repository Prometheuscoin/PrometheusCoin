...
int64 nChainStartTime = 1403606669;
...
const string strMessageMagic = "Prometheus Signed Message:\n";
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    if(nHeight <= 2)
	nSubsidy = 13750 * COIN; // first 50 blocks are 1 coin reward
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight)
{
    ...
    nRewardCoinYear = MAX_PMC
;

    if(nHeight < YEARLY_BLOCKCOUNT)
        nRewardCoinYear = 50 * MAX_PMC
;
    else if(nHeight < (2 * YEARLY_BLOCKCOUNT))
        nRewardCoinYear = 1 * MAX_PMC
;
...
static const int64 nTargetTimespan = 20 * 30; // 30 blocks  
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in Prometheus
    bool fStrictPayToScriptHash = true; // Always active in Prometheus
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Prometheus", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        assert(block.hashMerkleRoot == uint256("0x6ec52f5b478c40a8b366bfcb6ef39726c3095361258b5c76396c3b2a08e70b9c"));
...
unsigned char pchMessageStart[4] = { 0x3d, 0x2b, 0x3a, 0xd1 };
